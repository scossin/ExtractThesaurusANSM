...
class PDDI:
    ...
    def __init__(self, main_drug: str, plus_drug: str, between_main_and_plus_drug: str, description: List[str]):
        ...
        self.description_string: str = "".join(description).strip()
        try:
    ...
    def _extract_interaction_information(self, description: List[str]):
        ...
        for num_line, line in enumerate(description):
            severity_levels_found = [SeverityLevelFinding(num_line, line, severity_level)
                                     for severity_level in SEVERITY_LEVELS
                                     if line.startswith(severity_level.name)]
            if len(severity_levels_found) != 0:
        ...
        if len(severity_levels_found) == 0:  # there is only one severity level and it MUST exist (or I didn't detected it)
            raise PDDIdescriptionError(f"no severity level detected in this description: "
                                       f"{description}")

        severity_level_found: SeverityLevelFinding = severity_levels_found[0]
    ...
    def __pddi_was_manually_curated(self) -> bool:
    ...
    def __several_severity_levels_info_extract(self, severity_level_found: SeverityLevelFinding,
        ...
        if len(tuple_last_part) < 2:
            raise PDDIdescriptionError(f"can't find last course of action and mechanism of interaction in {last_part}"
                                       f"extracted from this pddi description: {description_string}")
    ...
    def __single_severity_level_info_extract(self, severity_level_found: SeverityLevelFinding,
    ...
    @classmethod
    def line_is_not_empty(cls, line: str) -> bool:
        return line.strip() != ""
    ...
    def __remove_plus_sign(cls, string):
        # remove only the first one: "+ OMBITASVIR + PARITAPRÉVIR" => "OMBITASVIR + PARITAPRÉVIR"
        string = re.sub(r"^[+]", "", string)
        return string

    @staticmethod
    def __extract_severity_info_lines(description: List[str], next_line: int) -> list:
    ...
    @staticmethod
    def __check_next_line_exists(next_line: int, description: List[str]) -> None:
