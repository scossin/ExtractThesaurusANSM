...
def extract_pddis(lines: list, debug_mode=False) -> list:
    ...
    lines_fixed: List[str] = list(map(fix_specific_lines_before_extraction, lines))
    if debug_mode:
        writeDebugFile(input_file, lines_fixed, 1.1)

    lines_filtered: List[str] = list(filter(lambda x: not is_a_line_2_ignore(x), lines_fixed))
    ...
    main_drugs: List[str] = [line for (tag, line) in tagged_lines
    ...
    drug_pddis_list: List[Any] = [None] * len(indices_main_drugs)
